...
@Component({
  ...
  template: `
    <div *ngIf="product(); else loadingTpl" class="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-fade-in">
      <!-- Image Gallery -->
      <div class="bg-white rounded-3xl p-8 border border-slate-100 flex items-center justify-center aspect-square">
        <img [src]="product()?.imageUrl" [alt]="product()?.name" class="max-h-full max-w-full object-contain hover:scale-105 transition-transform duration-500">
      </div>

      <!-- Info -->
      <div class="flex flex-col justify-center space-y-6">
        <div>
          <span class="text-primary font-medium text-sm tracking-wider uppercase bg-primary/5 px-3 py-1 rounded-full">
            {{ product()?.category }}
          </span>
          ...
          <div class="flex items-center gap-4">
            ...
            <span *ngIf="product()?.stock" class="text-green-600 text-sm font-medium flex items-center gap-1">
